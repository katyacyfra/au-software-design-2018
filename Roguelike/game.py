...
import os
import sys
from utilities import Spritesheet, re_roll, write
...
class Boss(Mob):
    def __init__(self, x, y, xp=0, level=1, hp=0, picture=""):
        ...
        self.sight_radius = 7
        self.picture = random.choice((Gameplay.DRAGON, Gameplay.SLENDERMAN, Gameplay.GOP,Gameplay.BOSS, Gameplay.DART ))
        self.hitpoints = 35
        self.hpmax = self.hitpoints
...
class Food(Item):
     def __init__(self, x, y):
         Item.__init__(self, x, y)
         self.picture = random.choice((Gameplay.PIZZA,Gameplay.COFFEE, Gameplay.MILK,Gameplay.CAKE, Gameplay.BURGER,  Gameplay.ICECREAM ))
...
class Loot(Item):
    def __init__(self, x, y, descr=""):
        Item.__init__(self, x, y)
        if descr == "":
            self.text = random.choice(["trash", "coin", "knife", "rags",
                                       "whiskas", "stone", "sword", "armor", "gem",
...
class Gameplay(object):
    ...
    def draw(self):
        ...
        #draw player
        self.screen.blit(self.player.picture, (Gameplay.scrollx + self.player.x * 32, Gameplay.scrolly +self.player.y*32))
        #GUI
...
def battle(attacker, defender, level):
    ...
    if attacker.hitpoints > 0 and defender.hitpoints > 0:
        ...
        if defender.y > attacker.y:
            ...
            fly_dy = -50
        Text_fly(defender.x, defender.y, "dmg: {}".format(damage), dx=fly_dx, dy=fly_dy)  # Text fly's away from opponent
        if blocked_damage > 0:
